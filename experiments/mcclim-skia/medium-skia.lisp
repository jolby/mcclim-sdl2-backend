...
(defun %default-paint-for-medium (medium)
  (multiple-value-bind (red green blue alpha)
      (clime::color-rgba (medium-ink medium))
    (let ((paint (skia-core::make-paint)))
      (skia-core::set-paint-color4f paint red green blue alpha)
      ;;xxx do we want the anti-alias setting?
      (skia-core::set-paint-anti-alias paint t)
      (skia-core::set-paint-stroke paint t)
      paint)))

(defun %sync-skia-paint-color-with-medium-ink (sk-color ink)
  (multiple-value-bind (red green blue alpha)
      (clime::color-rgba ink)
    (skia-core::set-color4f sk-color red green blue alpha)
    sk-color))

(defun %sync-skia-paint-styles-with-medium (medium)
  (let ((line-style (medium-line-style medium)))
    (skia-core::set-paint-stroke-width canvas::*paint* (line-style-thickness line-style))
    ))
...
(defmacro with-skia-canvas ((medium) &body body)
  (alx:once-only (medium)
    `(let ((canvas::*canvas* (%lookup-skia-canvas ,medium))
           ...
           ;; (canvas::*paint* (or canvas::*paint* (%default-paint-for-medium medium)))
           ;; (canvas::*paint* (%default-paint-for-medium medium))
           )
       (%sync-skia-paint-color-with-medium-ink canvas::*paint* (medium-ink medium))
       (%sync-skia-paint-styles-with-medium medium)
       (progn ,@body))))
